...
_TABLE_NOTE_LONG: dict[int, str] = {
    55: (
        ...
        "<p><b>Jakým matematickým aparátem se to počítalo.</b> Goniometrie byla v té době "
        "plně k dispozici — jen ne pod tímto jménem (slovo „trigonometria“ zavedl až "
        "B. Pitiscus r. 1595, osm let po tomto opisu). Sférická trigonometrie byla hotová od "
        "antiky (Ptolemaios, tětivy) a islámské astronomie (sinová věta, ~1000) a v Evropě ji "
        "soustavně podal <b>Regiomontanus, De triangulis omnimodis</b> (~1464, tisk 1533). "
        "Počítalo se z <b>tabulek</b> sinů, tangent („umbra“) a sečen — sinus byl <i>délka</i> "
        "(polovina tětivy) pro poloměr R (sinus totus), ne desetinná funkce; <b>logaritmy ještě "
        "nebyly</b> (Napier 1614), násobení a dělení mnohamístných hodnot se dělalo ručně, "
        "případně s pomocí <b>prosthaferéze</b> (převod součinu na součet — právě v 80. letech "
        "16. stol. ji používali Wittich, Brahe a v Kasselu/Praze Jost Bürgi). Klíčový vzorec je "
        "<b>ascensionální diference</b> (differentia ascensionalis) z pravoúhlého sférického "
        "trojúhelníku: <code>sin(AD) = tg φ · tg δ</code>, neboli <code>cos H = −tg φ · tg δ</code> "
        ...
        "<code>sin δ = sin ε · sin λ</code>; délku λ pro daný den dávaly <b>Alfonsinské tabulky</b> "
        "(13. stol., stále v užívání) nebo novější <b>Pruténské tabulky</b> (E. Reinhold 1551, "
        "kopernikovské), případně efemeridy. To, že tabule <b>nezahrnuje refrakci</b>, přesně "
        "odpovídá tomuto „holému“ vzorci (refrakci kvantifikoval až Tycho Brahe v téže době, "
        "ještě nebyla v běžných tabulkách).</p>"
        "<p><b>Spíš počítáno na papíře než odečteno z přístroje.</b> Astroláb (a orloj sám je "
        "v jádru astroláb) by východ pro pražskou šířku ukázal otočením sítě nad tympanem — "
        "jenže odečet má přesnost ~stupně (~4 min) a rozptyloval by se; naše těsnost (RMS "
        "0,8 min přes rok) takový odečet <b>vylučuje</b> jako zdroj tabulky. „Nomogram“ v "
        "moderním smyslu (zákrytové grafy) je anachronismus (konec 19. stol.); dobovou "
        "grafickou obdobou byl <b>analema</b> (ortografická projekce, Ptolemaios–Vitruvius), ale "
        "i ta dává nižší přesnost. Nejpravděpodobnější tedy je <b>ruční výpočet</b> z "
        "trigonometrických a deklinačních tabulek, s astrolábem/orlojem jako názorovým rámcem "
        "a případnou kontrolou. Rekonstrukce postupu pro každý den: (1) datum → délka Slunce λ "
        "(Alfonsinské/Pruténské tabulky), (2) λ → deklinace δ (tabulka deklinace), (3) "
        "AD ze <code>sin(AD) = tg φ · tg δ</code> (φ Prahy pevné), (4) východ = 6<sup>h</sup> − "
        "AD/15, zaokrouhleno na minutu; obdobně poledne a západ.</p>"
        "<p><b>Závěr.</b> Tabule východu (a obdobně poledne a západu) Slunce je <b>produktem "
        "ručního výpočtu</b>, ne měření ani odečtu z přístroje: ze sférické trigonometrie "
        "(ascensionální diference <code>cos H = −tg φ · tg δ</code>) a tabulek deklinace "
        "<i>pravého</i> Slunce počítá geometrický východ středu Slunce bez refrakce, pro Prahu, "
        "v juliánském kalendáři kalibrovaném na rovnodennost 21. března. Náš přepis se s tímto "
        "modelem shoduje na úrovni zaokrouhlení (RMS 0,8 min), takže je věrný a bez systematické "
        "chyby.</p>"
    ),
}
